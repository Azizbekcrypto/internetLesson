...
const QZ_BG_SHAPES = [
  { ch: 'http', l: 5,  t: 10, s: 42, c: 'rgba(20,17,14,0.05)',   d: 19, dl: 0 },
  { ch: 'DNS',  l: 85, t: 8,  s: 40, c: 'rgba(255,79,40,0.08)',  d: 23, dl: 1.5 },
  { ch: '🌐',   l: 8,  t: 72, s: 48, c: 'rgba(15,166,214,0.14)', d: 27, dl: 0.8 },
  { ch: '.uz',  l: 81, t: 68, s: 34, c: 'rgba(34,160,92,0.10)',  d: 21, dl: 2.2 },
  { ch: 'IP',   l: 45, t: 86, s: 30, c: 'rgba(20,17,14,0.05)',   d: 25, dl: 1.1 },
  { ch: 'www',  l: 66, t: 25, s: 26, c: 'rgba(15,166,214,0.09)', d: 17, dl: 0.4 },
  { ch: '404',  l: 25, t: 35, s: 26, c: 'rgba(255,79,40,0.09)',  d: 20, dl: 1.9 },
  { ch: '</>',  l: 92, t: 44, s: 24, c: 'rgba(20,17,14,0.05)',   d: 24, dl: 1.3 },
  { ch: '://',  l: 2,  t: 45, s: 24, c: 'rgba(20,17,14,0.045)',  d: 26, dl: 2.6 },
];
...
function QzFX() {
  ...
  useEffect(() => {
    ...
    const draw = (tm) => {
      ...
      ctx.lineWidth = 1 * DPR;
      for (let a = 0; a < em.length; a++) for (let b = a + 1; b < em.length; b++) { const dx = em[a].x - em[b].x, dy = em[a].y - em[b].y, d = Math.sqrt(dx * dx + dy * dy), mx = 95 * DPR; if (d < mx) { ctx.strokeStyle = 'rgba(15,166,214,' + (.06 * (1 - d / mx)) + ')'; ctx.beginPath(); ctx.moveTo(em[a].x, em[a].y); ctx.lineTo(em[b].x, em[b].y); ctx.stroke(); } }
      for (const p of em) { const s = (1.3 + p.z * 2.2) * DPR, tw = .16 + p.z * .24 + Math.sin(tm / 600 + p.ph) * .08; ctx.fillStyle = 'rgba(255,79,40,' + tw + ')'; ctx.beginPath(); ctx.arc(p.x, p.y, s, 0, 6.29); ctx.fill(); }
      ...
      <div className="screen">
        ...
        <div className={`qz-cta fade-up d2 ${studentLive ? 'ready' : ''}`}>
          <div className="qz-cta-txt">
            <span className="qz-cta-h"><QzBolt size={22} /> Mustahkamlash testi</span>
