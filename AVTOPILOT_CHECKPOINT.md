...
| `BotAiAgentLesson` | ❌ **v16, 95KB — Quruvchi faylga YOZMAGAN** (API xatosi). Hech narsa buzilmagan, noldan | → Quruvchi → Jonli → SAYQAL → Tekshiruvchi → Verifikator |
| `BotFeedbackIterationLesson` | **Quruvchi 🔵** (📔 TILAKLAR DAFTARI · voronka · yaxshilash aylanasi) | → Jonli → SAYQAL → Tekshiruvchi → Verifikator |
| `BotFullProjectLesson` | ⬜ v16 — boshlanmagan (🏠 DOIMIY JOY: deploy=ko'chirish, server=doim yoqilgan kompyuter) | → to'liq zanjir |
| `BotAiProjectLesson` | ⬜ v16 — boshlanmagan (⭐ capstone: hamma buyum birga) | → to'liq zanjir |
